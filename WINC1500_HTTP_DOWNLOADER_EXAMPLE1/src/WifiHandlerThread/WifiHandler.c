...
QueueHandle_t xQueueNitrogenBuffer = NULL;  ///< Queue to send the nitrogen content to the cloud
QueueHandle_t xQueuePhosphorusBuffer = NULL;  ///< Queue to send the phosphorus content to the cloud
QueueHandle_t xQueuePotassiumBuffer = NULL;  ///< Queue to send the potassium content to the cloud
QueueHandle_t xQueueMicBuffer = NULL;		///< Queue to send the microphone to the cloud
QueueHandle_t xQueueTempBuffer = NULL;		///< Queue to send the soil temperature to the cloud
QueueHandle_t xQueueMoistureBuffer = NULL;		///< Queue to send the soil moisture to the cloud
//QueueHandle_t xQueueSprinklerBuffer = NULL;		///< Queue to send the sprinkler status to the cloud
...
//static void MQTT_HandleSprinklerMessages(void);
...
{
	bool rel_out;
	if (pdPASS == xQueueReceive(xQueueSprinklerBuffer, &rel_out, 0)) {
		snprintf(mqtt_msg, 63, "%d", rel_out);
...
{
	int error = xQueueSend(xQueuePhosphorusBuffer, P, (TickType_t)10);
	return error;
}

int WifiAddPotassiumDataToQueue(uint16_t *K)
{
	int error = xQueueSend(xQueuePotassiumBuffer, K, (TickType_t)10);
	return error;
}
 ...
 * @brief	Adds an microphone data to the queue to send via MQTT
 * @param[out]
...
/**
 int WifiAddTempDataToQueue(float *val)
 * @brief	Adds an soil temperature data to the queue to send via MQTT
 * @param[out]
 ...
 * @brief	Adds an soil moisture data to the queue to send via MQTT
 * @param[out]
 ...
 * @brief	Adds an sprinkler (relay) data to the queue to send via MQTT
 * @param[out]
...
/*int WifiAddSprinklerDataToQueue(bool *rel_out)
{
	int error = xQueueSend(xQueueSprinklerBuffer, rel_out, (TickType_t)10);
	return error;
}*/
